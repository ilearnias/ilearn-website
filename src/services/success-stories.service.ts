...
export interface ISuccessStoryCreate
  extends Omit<ISuccessStory, "id" | "createdAt" | "updatedAt"> {}
...
class SuccessStoryService {
  ...
  async getPublicSuccessStories(
    ...
    try {
      return await apiRequest.get<ISuccessStory[]>(
        `${API_ENDPOINTS.PUBLIC.SUCCESS_STORIES}?page=${page}&limit=${limit}`
      );
    } catch (error) {
  ...
  async getSuccessStoryById(id: string): Promise<ApiResponse<ISuccessStory>> {
    try {
      return await apiRequest.get<ISuccessStory>(
        API_ENDPOINTS.ADMIN.SUCCESS_STORIES.DETAIL(id)
      );
    } catch (error) {
  ...
  async createSuccessStory(
    data: ISuccessStoryCreate
  ): Promise<ApiResponse<ISuccessStory>> {
    try {
      return await apiRequest.post<ISuccessStory>(
        API_ENDPOINTS.ADMIN.SUCCESS_STORIES.CREATE,
        data
      );
    } catch (error) {
  ...
  async updateSuccessStory(
    id: string,
    data: ISuccessStoryUpdate
  ): Promise<ApiResponse<ISuccessStory>> {
    try {
      return await apiRequest.patch<ISuccessStory>(
        API_ENDPOINTS.ADMIN.SUCCESS_STORIES.UPDATE(id),
        data
      );
    } catch (error) {
  ...
  async deleteSuccessStory(id: string): Promise<ApiResponse<null>> {
    try {
      return await apiRequest.delete<null>(
        API_ENDPOINTS.ADMIN.SUCCESS_STORIES.DELETE(id)
      );
    } catch (error) {
  ...
  async uploadSingleImage(file: File): Promise<string> {
    try {
      ...
      const response = await apiRequest.upload(
        API_ENDPOINTS.ADMIN.SUCCESS_STORIES.UPLOAD,
        formData
      );
