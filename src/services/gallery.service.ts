...
export const galleryService = {
  ...
  getPublicGalleryItems: async (page = 1, limit = 50, order = "ASC") => {
    try {
      const response = await apiRequest.get(
        `${API_ENDPOINTS.PUBLIC.GALLERY}?order=${order}&page=${page}&limit=${limit}`
      );
      if (!response.status) {
  ...
  getItemById: async (id: string) => {
    try {
      const response = await apiRequest.get(
        API_ENDPOINTS.ADMIN.GALLERY.ITEMS.DETAIL(id)
      );
      if (!response.status) {
  ...
  createItem: async (data: IGalleryItemCreate) => {
    try {
      const response = await apiRequest.post(
        API_ENDPOINTS.ADMIN.GALLERY.ITEMS.CREATE,
        data
      );
  ...
  updateItem: async (id: string, data: Partial<IGalleryItemCreate>) => {
    try {
      const response = await apiRequest.patch(
        API_ENDPOINTS.ADMIN.GALLERY.ITEMS.UPDATE(id),
        data
      );
  ...
  deleteItem: async (id: string) => {
    try {
      const response = await apiRequest.delete(
        API_ENDPOINTS.ADMIN.GALLERY.ITEMS.DELETE(id)
      );
      ...
      throw {
        ...
        message:
          error.message ||
          "Failed to delete gallery item. Please check your connection and try again.",
  ...
  uploadImages: async (files: File[], itemId?: string) => {
    try {
      ...
      const response = await apiRequest.upload(
        API_ENDPOINTS.ADMIN.GALLERY.UPLOAD,
        formData
      );
